...
const Authentication: React.FC<AuthenticationProps> = ({ staff, onAuthSuccess, onBack, theme }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className={`rounded-2xl shadow-2xl p-8 w-full max-w-md transition-colors ${
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className={`border rounded-lg p-4 text-center ${
              ...
              <div className={`text-xl font-mono min-h-[1.5rem] ${
                isDark ? 'text-gray-300' : 'text-gray-600'
              }`}>
                {pin.replace(/./g, '•')}
              </div>
            ...
            {keypadButtons.flat().map((button, index) => {
              if (button === 'clear') {
                return (
                  <button
                    ...
                    className={`col-span-1 font-semibold py-4 px-4 rounded-lg transition-colors text-sm ${
                      isDark 
              ...
              if (button === 'backspace') {
                return (
                  <button
                    key={index}
                    onClick={() => handleKeypadPress(button)}
                    className={`col-span-1 font-semibold py-4 px-4 rounded-lg transition-colors flex items-center justify-center ${
                      isDark 
              ...
              return (
                <button
                  ...
                  className={`font-semibold py-4 px-4 rounded-lg transition-colors text-xl disabled:cursor-not-allowed ${
                    isDark 
                      ? 'bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-600 text-gray-200' 
                      : 'bg-gray-100 hover:bg-gray-200 disabled:bg-gray-50 disabled:text-gray-400 text-gray-900'
                  }`}
                >
                  {button}
                </button>
